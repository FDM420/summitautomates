...
/**
 * All ISO 3166-1 alpha-2 countries and commonly used territories with their
 * primary E.164 dial code. Sorted alphabetically by `name`.
 *
 * The data source / business provider is referred to elsewhere only as the
 * "free tier" / "provider"; this module is purely a static reference list.
 */
 ...
 * regional-indicator code points. Returns a neutral white flag for invalid
 * input.
 */
...
/**
 * Looks up a country by its ISO alpha-2 code (case-insensitive). Returns
 * undefined when the code is unknown.
 */
...
/**
 * Resolves an ISO alpha-2 code to its display name, falling back to the
 * (upper-cased) code itself when no match is found.
 */
...
/**
 * Ready-to-render options for the country `Select`: value is the ISO code, label
 * is "<flag> <name>". Order matches `COUNTRIES` (alphabetical by name).
 */
